...
public class honestAgent extends Agent{
             ...
             public void action() 
             {
                ...
                if (msg!=null) {
                	if (receiver != null && msg.getSender().getName().equals(receiver.getName()) && (msg.getContent().equals("false") || msg.getContent().equals("true"))) {
                		...
                		// The honest agent is accepting propositions only when it's equally shared
                	...
                	if (nb_propositions == max_interractions && receiver == null) {
                		...
                		// The honest agent always split the points it has to share
                    	ACLMessage message = new ACLMessage(ACLMessage.INFORM);
	...
	protected void takeDown() {
    	...
    	ACLMessage message = new ACLMessage(ACLMessage.INFORM);
    	message.addReceiver(new AID("COORDINATOR", AID.ISLOCALNAME));
    	message.setContent("honestAgent" + " " + String.valueOf(totalPoints) + " " + String.valueOf(acceptedPropositions) + " " + String.valueOf(refusedPropositions));
    	send(message);
	}
}
