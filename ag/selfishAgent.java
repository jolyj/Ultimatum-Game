...
public class selfishAgent extends Agent{
             ...
             public void action() 
             {
                ...
                if (msg!=null) {
                	if (receiver != null && msg.getSender().getName().equals(receiver.getName()) && (msg.getContent().equals("false") || msg.getContent().equals("true"))) {
                		...
                		totalPoints += (msg.getContent().equals("true") ? (pointsPerProposition * 90) / 100 : 0);
                		...
                		// The selfish agent is accepting propositions only when it's equally shared
                		int proposition = Integer.parseInt(msg.getContent());
                		if (proposition >= pointsPerProposition / 2) {
                	...
                	if (nb_propositions == max_interractions && receiver == null) {
                		...
                		// The selfish agent always keep 90% for itself
                    	ACLMessage message = new ACLMessage(ACLMessage.INFORM);
                    	...
                    	message.setContent(String.valueOf(pointsPerProposition  - (pointsPerProposition * 90) / 100));
                    	send(message);
                    	nb_propositions += 1;
                	}
	...
	protected void takeDown() {
    	...
    	ACLMessage message = new ACLMessage(ACLMessage.INFORM);
    	message.addReceiver(new AID("COORDINATOR", AID.ISLOCALNAME));
    	message.setContent("selfishAgent" + " " + String.valueOf(totalPoints) + " " + String.valueOf(acceptedPropositions) + " " + String.valueOf(refusedPropositions));
    	send(message);
	}
}
