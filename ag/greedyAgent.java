...
public class greedyAgent extends Agent{
             ...
             public void action() 
             {
                ...
                if (msg!=null) {
                	if (receiver != null && msg.getSender().getName().equals(receiver.getName()) && (msg.getContent().equals("false") || msg.getContent().equals("true"))) {
                		...
                		// The greedy agent is accepting every proposition so it always answers true
                	...
                	if (nb_propositions == max_interractions && receiver == null) {
                		...
                		// The greedy agent always keeps 90% for itself
                    	ACLMessage message = new ACLMessage(ACLMessage.INFORM);
	...
	protected void takeDown() {
    	...
    	ACLMessage message = new ACLMessage(ACLMessage.INFORM);
    	message.addReceiver(new AID("COORDINATOR", AID.ISLOCALNAME));
    	message.setContent("greedyAgent" + " " + String.valueOf(totalPoints) + " " + String.valueOf(acceptedPropositions) + " " + String.valueOf(refusedPropositions));
    	send(message);
	}
}
